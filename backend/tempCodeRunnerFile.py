...
def load_onnx_model():
    ...
    model = ORTModelForFeatureExtraction.from_pretrained(ONNX_DIR.as_posix(), local_files_only=True)
    tokenizer = AutoTokenizer.from_pretrained(ONNX_DIR.as_posix(), local_files_only=True)
    return model, tokenizer

if USE_ONNX:
    model,tokenizer = load_onnx_model()
else:
...
activity_embedding_file_path = BASE_DIR / "ml_models" / "activity_embeddings.npy"
...
ACTIVITIES = np.load(activity_list_file_path, allow_pickle=True)
# Precompute embeddings using OpenAI
embeddings = get_embedding(ACTIVITIES)
